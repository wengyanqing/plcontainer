...
int PlcDocker_create(runtimeConfEntry *conf, char **name, char *uds_dir, int qe_pid, int session_id, int ccnt, int uid, int gid,int procid) {
    std::string id = PlcDocker::create(conf, std::string(uds_dir), qe_pid, session_id, ccnt, uid, gid, procid);
    if (id.length() == 0) {
        return -1;
    }
...
int PlcDocker_inspect(ContainerEntry** entries, int length) {
    ...
    PlcDocker::inspect_status(container_ids,status);
}
...
std::string PlcDocker::create(runtimeConfEntry *conf, std::string uds_dir,int qe_pid, int session_id, int ccnt, int uid, int gid,int procid) {
